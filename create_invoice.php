  ...
  <div class="container">
    ...
    <form id="invoice_form">
      ...
      <div class="form-group">
        <label>Patient Name <i class="text-danger">*</i></label>
      ...
      <div class="row">
        <div class="col-md-12">
          <table class="table table-bordered table-hover" id="tab_logic">
            <thead>
              <tr>
                <th width="30%" class="text-center">Test Name</th>
      ...
      <div class="row">
        <div class="col-md-12">
          <button type="button" class="btn btn-primary font-weight-bold" id="btnCreateInvoice"><i class="fas fa-save"></i> Save</button>
...
<script>
  ...
  $(document).ready(function(){
    ...
    $('#loadItemTableTemp').load("item_table_temp.php");
    ...
    function addItem(){
      ...
      if(error_customer_name == false && error_item_name == false && error_qty == false && error_price == false){
        ...
        $.ajax({
          type: "POST",
          data: $('#invoice_form').serialize()+'&action=add_item',
          url: "item_action.php",
    ...
    function createInvoice(){
      ...
      $.ajax({
        type:"POST",
        data: $('#invoice_form').serialize()+'&action=create_invoice',
        url:"item_action.php",
        dataType:"json",
        success:function(data){
          if(data.status=='success') {
            ...
            $('#invoice_sucess_message').html('<div class="alert alert-success">'+data.message+'</div>');
            $('#successMessageModal').modal('show');
            document.getElementById("customer_address").focus();
            $("#item_name").removeClass("is-invalid");
            $('#download_invoice_button').html('<a href="print_invoice.php?order_id='+data.order_id+'&output" class="btn btn-info" role="button" aria-pressed="true"><i class="fas fa-download"></i> Download</a>');
            $('#print_invoice_button').html('<a href="print_invoice.php?order_id='+data.order_id+'" class="btn btn-info" role="button" aria-pressed="true" target="_blank"><i class="fas fa-print"></i> Print</a>');
    ...
    $('#btn_cancel').click(function(){
      $.ajax({
        ...
        success: function(data) {
          ...
          $('#loadItemTableTemp').load("item_table_temp.php");
          $('#sucess_message').show();
          $('#sucess_message').html('<div class="alert alert-success"><i class="fas fa-check"></i> Successfully Cancel</div>');
          setTimeout(function () {
            $('#sucess_message').hide();
          }, 2000);
        },
  ...
  function eliminateItem(index) {
    $.ajax({
        type: "POST",
        data: {action:'eliminate_item', index:index},
        url:"item_action.php",
        success: function(data) {
          $('#loadItemTableTemp').load("item_table_temp.php");
        },
